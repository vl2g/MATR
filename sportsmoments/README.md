...
We release all the Youtube Video IDs in the `vids.txt` file. For downloading the raw query and reference clips, please visit the following [link](https://huggingface.co/datasets/yogeshiitj/SportsMoment/tree/main)

Our annotation files consist of 3 splits : `train`, `val` and `test`. You can find all the files in the link above. Each file has the following fields :
```
...
```
We release the `id_class_mapping.json` for mapping the `action_class` to its corresponding class name. In addition to the annotation files, we also provide the visual captions associated with each of the query videos [here](https://huggingface.co/datasets/yogeshiitj/SportsMoment/tree/main), if one wishes to evaluate `MATR` on textual queries.
