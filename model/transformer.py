...
class Transformer(nn.Module):
    ...
    def __init__(self, d_model=512, nhead=8, num_encoder_layers=6,
        ...
        # layer to project (bsz, L_vid, d) to (bsz, 10, d)
        # self.vid_proj = nn.Linear(vid_input.shape[1], 10)

        decoder_norm = nn.LayerNorm(d_model)
        self.decoder = TransformerDecoder(decoder_layer, num_decoder_layers, decoder_norm,
                                          return_intermediate=return_intermediate_dec)
        
        self.vid_proj = nn.Conv1d(d_model, 10, kernel_size=1)
    ...
    def forward(self, src, mask, query_embed, pos_embed, src_vid):
        """
        Args:
            src: (batch_size, L, d)
            mask: (batch_size, L)
        ...
        # batch_size, x, d = src_vid.shape
        
        # # # Permute to (batch_size, d, x) for Conv1d
        # src_vid = src_vid.permute(0, 2, 1)
        
        # # Apply Conv1d
        # projected_tensor = self.vid_proj(src_vid)  # (batch_size, 15, d)
        # print(f"projected_tensor: {projected_tensor.shape}")
        # # Permute to (10, batch_size, x)
        # projected_tensor = projected_tensor.permute(1, 0, 2)  # (10, batch_size, x)
        ...
        # print(vid_mem.shape)
        # print(mask[:,:l].shape)
        # print(pos_embed[:l,:,:].shape)
        
        hs = self.decoder(tgt, vid_mem, memory_key_padding_mask=mask[:,:l],
