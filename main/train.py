...
def train_epoch(model, criterion, train_loader, optimizer, opt, epoch_i, tb_writer):
    ...
    for batch_idx, batch in tqdm(enumerate(train_loader),
        ...
        alignment_loss1,_ = compute_alignment_loss(
        samples=samples, 
        ...
        alignment_loss2,_ = compute_alignment_loss(
        samples=samples, 
        distractors=None,        # No distractors in this case
        l2_normalize=True,       # Normalize features if needed
        # drop_cost_type='max',    # Choose how to compute cost between sequences
        ...
        losses = sum(loss_dict[k] * weight_dict[k] for k in loss_dict.keys() if k in weight_dict) + alignment_loss1 + alignment_loss2
